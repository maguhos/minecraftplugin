 ...
 * <br><br>
 * Handles arguments, argument type handling, predictions, tab completion,
 * sub-sub-sub-commands, error handling, help pages, and more!
 * <br><br>
 * CommandManager manager = new CommandManager(plugin, "Help Tag", "permission
 * node","command");
 *
 * @author Stumblinbear
...
public class CommandManager implements TabCompleter, CommandExecutor {
    ...
    /* extends the class and add the Cmd annotation 
    @Cmd(cmd = "debug",
            args = "[bool]",
    ...
    public CommandManager(JavaPlugin plugin, String desc, String permissionScheme, String command, String... aliases) {
        this.tag = command + " command";
        this.desc = desc;
        ...
        try {
            ...
            if (k != null) {
                pluginCommand = (PluginCommand) k;
                pluginCommand.unregister(commandMap);
            } else {
                Constructor<PluginCommand> c = PluginCommand.class.getDeclaredConstructor(String.class, Plugin.class);
                c.setAccessible(true);
                pluginCommand = c.newInstance(command, plugin);
            }
            pluginCommand.setTabCompleter(this);
            pluginCommand.setExecutor(this);
            pluginCommand.setDescription(desc);
            pluginCommand.setUsage("/" + command + " help for more info");
            if (aliases.length > 0) {
    ...
    private boolean matchfirstpart(Cmd c, String[] args) {
        String[] cmdArg = c.cmd().split(" ");
        int l = cmdArg.length - 1;
        if (l > args.length) {
            l = args.length;
        }
        for (int i = 0; i < l; i++) {
            System.out.println(args[i] + ' ' + cmdArg[i]);
            if (!args[i].toLowerCase().equalsIgnoreCase(cmdArg[i])) {
                return false;
            }
        }
        return true;
    ...
    public CommandFinished displayHelp(CommandSender sender, String[] args) {
        ...
        int perPage = 7; // How many commands to show per page
        ...
        if (helpList.size() >= page * perPage) {
            for (int j = 0; j < perPage; j++) {
                if (helpList.size() > (j + page * perPage)) {
                    if (j == 0) {
                        String header = ChatColor.YELLOW + "--------- " + ChatColor.WHITE + tag + " help (" + (page + 1) + "/" + (int) Math.ceil(helpList.size() / (perPage * 1F)) + ")" + ChatColor.YELLOW + " -----------------------------------------------------------------";
                        sender.sendMessage(" ");
                        sender.sendMessage(header.substring(0, 60));
                    }
    ...
    /**
     * Returns the string version of an exception. Helps with in-game error
     * checking.
     */
    ...
    /**
     * Attach to a function to denote it as a command. You must register the
     * class it is in as a command class before it will be used.
     */
    public static @interface Cmd {

        /**
         * Takes in any amount of subcommands. The command handler always
         * chooses the best-fit command.
         * <br><br>
         * Example:
         * <br>
        ...
        /**
         * The arguments for the command. Required arguments must be enclosed in
         * <>'s
         * <br><br>
         * Example:
         * <br>
        ...
        /**
         * Specifies the type that an argument should be. Default is
         * Arg.ArgString.
         * <br><br>
         * Example:
         * <br>
        ...
        /**
         * Specifies if the command should be restricted to CONSOLE, OP, or
         * PLAYERS. Otherwise, ALL.
         */
         ...
         * permission = "edit"
         * <br>
         * This setup would require a player to have testnode.edit to use the
         * command.
         */
    ...
    public static enum CommandOnly {
        ...
        PLAYER,
        /**
         * Only op players can use the command. Can be overridden by a matched
         * permission.
         */
        OP,
        /**
    ...
    /**
     * This can be used to immediately throw an error without returning a
     * <code>CommandFinished</code>. It'll display the specified error.
     */
    ...
    public static interface IArgParse<T> {
        ...
        List<String> getPredictions(String arg);
    ...
    public static class Arg {
        ...
        public static class ArgArray extends AbstractArg<List<String>> {
            ...
            public List<String> getPredictions(String arg) {
                return null;
            }
        }
        ...
        public static class ArgBoolean extends AbstractArg<Boolean> {
            ...
            public List<String> getPredictions(String arg) {
                if (arg.length() > 1 && f.startsWith(arg.toLowerCase())) {
                    return Arrays.asList(new String[]{"false"});
                }
                if (arg.length() > 1 && t.startsWith(arg.toLowerCase())) {
